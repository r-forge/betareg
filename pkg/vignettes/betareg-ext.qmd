...
`x2`, and `x3` to the mean submodel~@eq-link1
and `z1` and `z2` to the precision
submodel @eq-link2, respectively.  Function `betareg()`
...
and **car** [@betareg:Fox+Weisberg:2011], e.g., `lrtest()`,
...
for the precision parameters as shown in @eq-link2.
...
Section~\@ref(sec-iteration) are closed-form expressions for the vector
...
as detailed in Section~\@ref(sec-iteration). To control the details of
...
The MOB framework of @betareg:Zeileis+Hothorn+Hornik:2008 is
...
d_{2,i}\left\{ \mu_i\left(\bar{T}_i -
  ...
  references, see
  @betareg:Zeileis+Hothorn+Hornik:2008\footnote{An example of
...
Package **flexmix** \citep{betareg:Leisch:2004,
...
@betareg:Cribari-Neto+Zeileis:2010. Figure~\@ref(fig:ReadingSkills),
...
```{r ReadingSkills, echo=FALSE, fig.width=6, fig.height=5.5}
data("ReadingSkills", package = "betareg")
...
```
\caption{{#fig:ReadingSkills} Reading skills data from @betareg:Smithson+Verkuilen:2006:
...
\end{tabular}
\caption{{#tab:ReadingSkills-bias} Comparison of coefficients and
  standard errors (in parentheses) in the interaction model for reading
  skills. The ML estimator from `rs_ml`, the BC estimator from
  `rs_bc`, and the BR estimator from `rs_br` all give
...
```{r ReadingSkills-phi-plot, fig.height=6.5, fig.width=6.5, echo=FALSE}
...
```
\caption{{#fig:readingskillsbias} Scatterplots of the logarithm
...
empirical analyses, @sec-gasoline replicates the results
...
```{r ReadingSkills-tree3, echo=TRUE}
plot(rs_tree)
```
...
```{r ReadingSkills-tree-plot, fig.height=7, fig.width=10, echo=FALSE}
plot(rs_tree)
```
\caption{{#fig:betatree} Partitioned beta regression model for the
  `ReadingSkills` data.}
\end{figure}
...
```{r ReadingSkills-mix, echo=TRUE, eval=FALSE}
...
```{r ReadingSkills-mix2, echo=FALSE, results='hide'}
...
```{r ReadingSkills-betamix-plot1, eval = FALSE, echo=FALSE}
...
\centering
\setkeys{Gin}{width=\textwidth}
```{r ReadingSkills-betamix-plot3, echo=FALSE, fig.height=5.5, fig.width=10}
par(mfrow = c(1, 2))
ix <- as.numeric(ReadingSkills$dyslexia)
...
```
\caption{{#fig:betamix} Fitted regression lines for the mixture
  ...
  the observed `dyslexic` variable where nondyslexic and dyslexic
...
## Bias correction/reduction for gasoline yield data {#sec-gasoline} {.appendix}
...
\end{tabular}
\caption{{#tab:GasolineYield-bias} ML, BC and BR estimates and
...
\end{tabular}
\caption{{#tab:GasolineYield-bias2} ML, BC and BR estimates and
  corresponding estimated standard errors (in parentheses) for a
